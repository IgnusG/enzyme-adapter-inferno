...
describe('mount', () => {
  ...
  describe('.find(selector)', () => {
    ...
    it('should work on non-single nodes', () => {
      const wrapper = mount(<div className="a">
          <div className="b">
  ...
  describe('.findWhere(predicate)', () => {
    ...
    it('should return shallow rendered string when debug() is called', () => {
      ...
      const content = 'blah';
      const wrapper = mount(<Foo data={content} />);
      expect(wrapper.debug()).to.equal(`<Foo data="${content}">
  ...
  describe('.setProps(newProps[, callback])', () => {
    ...
    it('should throw if an exception occurs during render', () => {
      class Trainwreck extends Component {
        render() {
  ...
  describe('.text()', () => {
    ...
    it('should handle simple text nodes', () => {
      const wrapper = mount(<div>some text</div>);
  ...
  describe('.props()', () => {
    it('should return the props object', () => {
      const fn = () => ({});
      const wrapper = mount(<div id="fooId" className="bax" onClick={fn} >
          <div className="baz" />
          <div className="foo" />
        </div>);
  ...
  describe('.parents([selector])', () => {
    ...
    it('should optionally allow a selector', () => {
      const wrapper = mount(<div className="bax foo">
          <div className="foo">
            <div className="bar">
              <div className="baz" />
            </div>
  ...
  describe('.reduce(fn[, initialValue])', () => {
    ...
    it('should accumulate a value', () => {
      ...
      const result = wrapper.find('.foo').reduce((obj, n) => {
        obj[n.prop('id')] = n.prop('className'); // eslint-disable-line no-param-reassign
        return obj;
      }, {});
  ...
  describe('.matchesElement(node)', () => {
    it('should match on a root node that looks like the rendered one', () => {
      ...
      expect(wrapper.matchesElement(<div>
          <div onClick={spy} style={{ fontSize: 12, color: 'red' }}>Hello World</div>
        </div>)).to.equal(true);
      expect(wrapper.matchesElement(<div>
  ...
  describe('.containsMatchingElement(node)', () => {
    ...
    it('should match on a single node that looks like a rendered one', () => {
      ...
      expect(wrapper.containsMatchingElement(<div onClick={spy1} style={{ fontSize: 12, color: 'red' }}>Hello World</div>)).to.equal(true);
      expect(wrapper.containsMatchingElement(<div style={{ fontSize: 12, color: 'red' }}>Hello World</div>)).to.equal(true);
      expect(wrapper.containsMatchingElement(<div onClick={spy2} style={{ fontSize: 13, color: 'blue' }}>Goodbye World</div>)).to.equal(true);
      expect(wrapper.containsMatchingElement(<div onClick={spy2}>Goodbye World</div>)).to.equal(true); // eslint-disable-line max-len
      expect(spy1.callCount).to.equal(0);
      expect(spy2.callCount).to.equal(0);
    });
    ...
    it.skip('should not match on a single node that doesn\'t looks like a rendered one', () => {
      ...
      expect(wrapper.containsMatchingElement(<div onClick={spy2}>Au revoir le monde</div>)).to.equal(false); // eslint-disable-line max-len
    });
  ...
  describe('.containsAllMatchingElements(nodes)', () => {
    it('should throw TypeError if non-array passed in', () => {
      ...
      expect(() => wrapper.containsAllMatchingElements(<div>
          Hello
        </div>)).to.throw(TypeError, 'nodes should be an Array');
    });
